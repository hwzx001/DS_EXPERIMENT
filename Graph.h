...
template<typename T>
inline void Graph<T>::insert_ad_v(int id, int ad_v)
{
	for (int i = 0;i < node_num;i++)
	{
		if (node[i].id == id)
		{
			if (find(node[i].ad_v.begin(), node[i].ad_v.end(), ad_v) == node[i].ad_v.end()) node[i].ad_v.push_back(ad_v);
			break;
		}
	}
}
...
template<typename T>
inline void Graph<T>::insert_v(int id, T data_x, T data_y, vector<int>ad_v)
{
	Vertex<T> temp;
	temp.id = id;
	temp.data_x = data_x;
